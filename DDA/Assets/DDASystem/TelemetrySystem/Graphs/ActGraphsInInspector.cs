...
public class ActGraphsInInspector : MonoBehaviour
{
    ...
    void Update()
    {
        configs = GetComponents<GraphConfig>();
        //Recorrido de todas las graficas del editor
        for (int i = 0; i < configs.Length; i++)
        {
            AnimationCurve anC = configs[i].data.myCurve;
            if (anC == null)
                continue;
            //Si no hay ningun segmento en x se anade un punto inicial a la grafica
            if (anC.keys.Length == 0)
            {
                Keyframe k = new Keyframe(0, 0);
                anC.AddKey(k);
                SetKeyRightTangentMode(anC, 0, TangentMode.Linear);
                SetKeyLeftTangentMode(anC, 0, TangentMode.Linear);
            }
            else
            {
                //Actualizacion de todos los puntos de la grafica
                for (int j = 0; j < configs[i].data.pointsNumber; j++)
                {
                    ...
                    anC.AddKey(k);
                    SetKeyRightTangentMode(anC, j, TangentMode.Linear);
                    SetKeyLeftTangentMode(anC, j, TangentMode.Linear);
                }
                //Elimino los puntos que sobran si se ha reducido pointsNumber
                while (anC.length > configs[i].data.pointsNumber)
